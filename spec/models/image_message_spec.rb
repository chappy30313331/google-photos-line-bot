...
RSpec.describe ImageMessage, type: :model do
  ...
  it 'returns message hash' do
    ...
    expect(message[:originalContentUrl]).to match(%r{https://lh3\.googleusercontent\.com/lr/.*})
    expect(message[:previewImageUrl]).to match(%r{https://lh3\.googleusercontent\.com/lr/.*})
  end
end
